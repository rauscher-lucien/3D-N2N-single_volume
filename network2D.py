""" Contains the 2D U-Net for denoising """
...
class Noise2NoiseUNet2D(nn.Module):
    """
...
class WUNet2D(nn.Module):
    """
        WU-Net: Wavelet U-Net for Medical Image Segmentation
        https://arxiv.org/abs/1505.04597
        https://link.springer.com/chapter/10.1007/978-3-030-61609-0_63
    ...
    def __init__(self, in_channels, out_channels, f_maps=16, num_groups=8, is_N2V2_setup=False, **kwargs):
        ...
        for i, out_feature_num in enumerate(f_maps):
            if i == 0:
                ...
                encoder = Encoder(f_maps[i - 1], out_feature_num, basic_module=DoubleConv,
                                    conv_layer_order=conv_layer_order, num_groups=num_groups, pool_type='dwt')
        ...
        for i in range(len(reversed_f_maps) - 1):
            ...
            out_feature_num = reversed_f_maps[i + 1]
            decoder = Decoder(in_feature_num, out_feature_num, basic_module=DoubleConv,
                              conv_layer_order=conv_layer_order, num_groups=num_groups, is_wunet=True)
            decoders.append(decoder)
...
# Generate MaxBlurPool2D for the N2V2 approach
class MaxBlurPool2d(torch.nn.Module):
    ...
    def __init__(self, pool_kernel_size, channels, blur_kernel=None):
        ...
        self.max_pool2d = nn.MaxPool2d(kernel_size=self.pool_kernel_size,
                                       stride=1, 
...
# Wavlet Unet components (see https://github.com/jeya-maria-jose/Unet_DWT/blob/master/unet/blocks.py)
# Generate Discrete Wavelet Transformation as downsampling layer
class DWT2d(torch.nn.Module):
    """
        Module for Downsampling in Encoder Path
    """

    def __init__(self) -> None:
        super().__init__()

        # Since the Wavlet package operates on numpy and on cpu, we need to switch off the gradient
        # For 2D pytorch versions look into https://github.com/fbcotter/pytorch_wavelets
        self.requires_gad = False
        # Check if GPU is available
        self.device = torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu")

    def __dwt2D(self, x):
        LL, _ = pywt.dwt2(x.detach().cpu().numpy(), 'db2')

        return torch.tensor(LL).to(self.device)

    def forward(self, x):
        return self.__dwt2D(x)

# Generate Inverse Discrete Wavelet Transformation as upsampling layer
class IDWT2d(torch.nn.Module):
    """
        Module for Upsampling in Decoder Path
    """
    ...
    def __idwt2D(self, x):
        # Should have following shape (a,(b,c,d))
        coef = x.detach().cpu().numpy(), (None, None, None)

        return torch.tensor(pywt.idwt2(coef, 'db2')).to(self.device)

    def forward(self, x):
        return self.__idwt2D(x)
    
# Usually for upsampling only the function is needed
def idwt2D(x, device):
    # Should have following shape (a,(b,c,d))
    coef = x.detach().cpu().numpy(), (None, None, None)

    return torch.tensor(pywt.idwt2(coef, 'db2')).to(device)
...
class Encoder(nn.Module):
    ...
    Args:
        ...
        pool_type (str): pooling layer: 'max' 'maxblur', 'dwt' or 'avg'
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 pool_kernel_size=(2, 2), pool_type='max', basic_module=DoubleConv, conv_layer_order='cr',
                 num_groups=8):
        super(Encoder, self).__init__()
        assert pool_type in ['max', 'maxblur', 'dwt', 'avg']
        if apply_pooling:
            if pool_type == 'max':
                self.pooling = nn.MaxPool2d(kernel_size=pool_kernel_size)
            elif pool_type == 'maxblur':
                self.pooling = MaxBlurPool2d(pool_kernel_size=pool_kernel_size, channels=in_channels)
            elif pool_type == 'dwt':
                self.pooling = DWT2d()
            else:
...
class Decoder(nn.Module):
    ...
    Args:
        ...
        conv_layer_order (string): determines the order of layers
            in `DoubleConv` module. See `DoubleConv` for more info.
        num_groups (int): number of groups for the GroupNorm
        is_wunet (bool): Flag, whether WU-Net should be generated
    """

    def __init__(self, in_channels, out_channels, kernel_size=3,
                 scale_factor=(2, 2), basic_module=DoubleConv, conv_layer_order='cr', num_groups=8, is_wunet = False):
        super(Decoder, self).__init__()

        self.is_wunet = is_wunet
        self.device = torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu")
    ...
    def forward(self, encoder_features, x):
        if self.upsample is None:
            ...
            if type(encoder_features) is torch.Size:
                if self.is_wunet:
                    # If it is a WU-net utilize the inverse DWT for upsampling 
                    x = idwt2D(x, self.device)
                # Sometimes dimension don't fit with original encoder features, adapt then
