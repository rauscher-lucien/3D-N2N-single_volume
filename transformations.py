...
class N2V_mask_generator(object):
    ...
    def __call__(self, data):
        ...
        for c in range(self.n_chan):
            ...
            input_val = value_manipulation(input[..., c], coords, self.dims, self.structN2Vmask)


            if self.structN2Vmask is not None:
                input_val_all = np.array(input_val)

                input_val = np.array(input_val_all)[:, 0]
                min_max_vals = np.array(input_val_all)[:, 1:]
            ...
            if self.structN2Vmask is not None:
                self.apply_structN2Vmask(input[..., c], coords, min_max_vals, self.structN2Vmask)
    ...
    def pm_uniform_withCP(self):
        def random_neighbor_withCP_uniform(patch, coords, dims, structN2Vmask):
            vals = []
        ...
        def patch_mean(patch, coords, dims, structN2Vmask=None):
            ...
            for coord in zip(*coords):
                ...
                if structN2Vmask is None:
                    ...
                    vals.append([np.mean(sub_patch[sub_patch_mask]),
                                np.min(sub_patch[sub_patch_mask]),
                                np.max(sub_patch[sub_patch_mask])])
        ...
        def patch_median(patch, coords, dims, structN2Vmask=None):
            ...
            for coord in zip(*coords):
                ...
                if structN2Vmask is None:
                    ...
                    #print(sub_patch[sub_patch_mask].shape)
                    vals.append([np.median(sub_patch[sub_patch_mask]),
                                np.min(sub_patch[sub_patch_mask]),
                                np.max(sub_patch[sub_patch_mask])])
    ...
    def apply_structN2Vmask(self, patch, coords, min_max_vals, mask):
        """
        ...
        #print(dx.shape)
        #print(mix.shape)
        #print(mix.transpose([1,0,2]).shape)
        ...
        ## replace neighbouring pixels with random values from values ranging between max and min in the picking window
        patch[tuple(mix.T)] = np.multiply(np.random.rand(mix.shape[0]), np.diff(min_max_vals, axis=-1).repeat(dx.shape[1])) - min_max_vals[:, 0].repeat(dx.shape[1])
        #patch[tuple(mix.T)] = np.multiply(np.random.rand(mix.shape[0]), np.diff(min_max_vals, axis=-1)[..., None]) - min_max_vals[:, 0]
