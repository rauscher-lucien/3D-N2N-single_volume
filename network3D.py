...
class DWT3d(torch.nn.Module):
    ...
    def __dwt3D(self, x):
        aaa = pywt.dwtn(x.detach().cpu().numpy(), 'db2', axes=[-3,-2,-1])['aaa']

        return torch.tensor(aaa).to(self.device)

    def forward(self, x):
        return self.__dwt3D(x)
...
class IDWT3d(torch.nn.Module):
    ...
    def __idwt3D(self, x):
        # Should be dict as follows, then the rest will be filled with Nones
        coef = {'aaa': x.detach().cpu().numpy()}

        return torch.tensor(pywt.idwtn(coef, 'db2',  axes=[-3,-2,-1])).to(self.device)

    def forward(self, x):
        return self.__idwt3D(x)
...
def idwt3D(x, device):
    ...
    return torch.tensor(pywt.idwtn(coef, 'db2',  axes=[-3,-2,-1])).to(device)
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
                 scale_factor=(2, 2, 2), basic_module=DoubleConv, conv_layer_order='cr', num_groups=8, is_wunet = False):
